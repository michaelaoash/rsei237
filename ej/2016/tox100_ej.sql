...
FROM release_cl r LEFT JOIN rsei237gma.release_ej_2016 j USING (ReleaseNumber),
...
SELECT 'begin ej_237_2016_chemical' AS ``;
...
SELECT 'end ej_237_2016_chemical' AS `` ;
...
SELECT 'begin ej_237_2016_industry' AS ``;
...
ORDER BY score_rev DESC,ratio_nonwhite DESC ;
SELECT 'end ej_237_2016_industry' AS `` ;
...
CREATE TEMPORARY TABLE ToxicCompanies_2016
SELECT @rownum:=@rownum+1 AS tox100_rank,c.* FROM ToxicCompanies_2016_temp c,(SELECT @rownum:=0) r  ORDER BY Score DESC ;

CREATE TEMPORARY TABLE Toxic100_2016
SELECT @rownum:=@rownum+1 AS tox100_rank,c.* FROM ToxicCompanies_2016_temp c,(SELECT @rownum:=0) r ORDER BY Score DESC LIMIT 100 ;
...
FROM Toxic100_2016,(SELECT @rownum:=0) r ORDER BY ratio_nonwhite DESC ;

SELECT @rownum:=@rownum+1,final_parent,tox100_rank,FORMAT(Facilities,0),FORMAT(Releases,0),FORMAT(Score,0),FORMAT(pounds_air_rev,0),FORMAT(pounds_incin_rev,0),FORMAT(ratio_nonwhite,4),FORMAT(ratio_black,4),FORMAT(ratio_hisp,4),FORMAT(ratio_poor,4),FORMAT(ratio_near_poor,4) 
-- FORMAT(ratio_a_pac,4),FORMAT(ratio_indian,4),
FROM Toxic100_2016,(SELECT @rownum:=0) r ORDER BY ratio_poor DESC ;

SELECT @rownum:=@rownum+1,final_parent,tox100_rank,FORMAT(Facilities,0),FORMAT(Releases,0),FORMAT(Score,0),FORMAT(pounds_air_rev,0),FORMAT(pounds_incin_rev,0),FORMAT(ratio_nonwhite,4),FORMAT(ratio_black,4),FORMAT(ratio_hisp,4),FORMAT(ratio_poor,4),FORMAT(ratio_near_poor,4) 
-- FORMAT(ratio_a_pac,4),FORMAT(ratio_indian,4),
FROM Toxic100_2016,(SELECT @rownum:=0) r ORDER BY Score DESC ;
...
FROM score_fac f, ToxicCompanies_2016 p
WHERE f.rpid=p.rpid
...
MAX(nogrid) AS nogrid
FROM score_chem_company c, ToxicCompanies_2016 p
WHERE c.rpid=p.rpid
GROUP BY rpid,cas
...
SELECT 'begin ej_237_2016_company-facility' AS `` ;
...
ORDER BY parent_score DESC, facility_score DESC, final_parent ;
SELECT 'end ej_237_2016_company-facility' AS `` ;


-- chemical: Air Scores by Parent, and within Parent by Chemical, Toxic 100' ;
SELECT 'begin ej_237_2016_company-chemical' AS `` ;
...
ORDER BY parent_score DESC, chemical_score DESC, chemical ;
SELECT 'end ej_237_2016_company-chemical' AS `` ;
...
SELECT *,(ISNULL(score_1) AND score_rev>0) AS nogrid FROM release_cl r LEFT JOIN rsei237gma.release_ej_2016 j USING (releasenumber) 
WHERE r.year=2016
...
SELECT 'begin ej_237_2016_company-facility-chemical' AS `` ;
...
SELECT 'end ej_237_2016_company-facility-chemical' AS `` ;
